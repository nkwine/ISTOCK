...
class make_sale:
    def __init__(self,root,parent,on_clos=None):
        ...
        #heading
        self.titlelable =Label(self.root,text="Make a sale",font=("times new roman",20,'bold'),bg="#0f4d7d",fg="white")
        ...
        self.name = Entry(headl,font=("times new roman",12,"bold"),bg="lightyellow",width=21)
        self.name.grid(row =0,column =1)
        contact = Label(headl,text="Contact(optional)",font=("times new roman",14,"bold"),bg="white")
        contact.grid(row =0,column =2,)
        self.contact = Entry(headl,font=("times new roman",12,"bold"),bg="lightyellow",width=15)
        self.contact.grid(row =0,column =3,)
        #middle
    ...
    def billup(self):
        ...
        if self.contact.get().strip()!="":
            if len(self.contact.get().strip())!=10 or self.contact.get().strip()[0]!="0":
        ...
        table = '''CREATE TABLE IF NOT EXISTS sales(s_id VARCHAR(20) PRIMARY KEY,date VARCHAR(30) NOT NULL,
        customer VARCHAR(200) NOT NULL,bill VARCHAR(200) NOT NULL,deposit VARCHAR(15) NOT NULL,
        balance VARCHAR(30),status VARCHAR(1000) NOT NULL)'''
        ...
        self.bill.insert(1.0,f"Sale Number: {new_id}\n")
    ...
    def inserti(self):
        ...
        mycursor = con.cursor()
        # try:
        query = "INSERT INTO sales VALUES(%s,%s,%s,%s,%s,%s,%s)"
        if int(to_insert_list[1].replace(",",""))>int(to_insert_list[3].replace(",","")):
        ...
        values =(to_insert_list[0],datetime.datetime.now().strftime("%m/%d/%Y"),self.name.get(),to_insert_list[1],to_insert_list[2],
                to_insert_list[3],status)
        if messagebox.askyesno("","Do you want to save this Sale"):
            mycursor.execute(query,values)
        else:
            return
        con.commit()
        table = '''CREATE TABLE IF NOT EXISTS sale_items(id INT PRIMARY KEY AUTO_INCREMENT,p_id VARCHAR(20) NOT NULL,
        s_id VARCHAR(20) NOT NULL,qty VARCHAR(200) NOT NULL,unit_measure VARCHAR(200),FOREIGN KEY(p_id)REFERENCES products(p_id),
        FOREIGN KEY(s_id)REFERENCES sales(s_id))
                '''
        mycursor.execute(table)
        for key,value in info1.items():
            mycursor.execute("SELECT qty,p_id,u_measure FROM products WHERE name =%s",key)
            record = mycursor.fetchone()
            query ="INSERT INTO sale_items(p_id,s_id,qty,unit_measure) VALUES((SELECT p_id FROM products WHERE name =%s),%s,%s,%s)"
            mycursor.execute(query,(key,to_insert_list[0],value[1],record[2]))
        ...
        file = open(f"{non_os_partition}:/inventory/sales/{to_insert_list[0]}.txt",'w')
        file.write(self.bill.get(1.0,END))
        file.close()
        messagebox.showinfo("",f"Sale saved successfuly",parent=self.root) 
        self.root.destroy()
