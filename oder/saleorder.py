from tkinter import *
import pymysql,platform,os,sys,datetime
from tkinter import ttk,messagebox
from tkcalendar import DateEntry
from .add_s_odder import sale_odder
class saleOrder:
    def __init__(self,root,parent):
        ...
        #heading
        self.titlelable =Label(self.root,text="Manage Sales Orders",font=("times new roman",20,'bold'),bg="#0f4d7d",fg="white")
        ...
        self.combo_box = ttk.Combobox(self.top_frame,values=("Order_No","date","customer","status"),font=("times new roman",12,"bold"),state="readonly")
        ...
        #op3
        self.status = ttk.Combobox(self.top_frame,values=("pendding","delivered","Over due","partially delivered","canceled"),font=("times new roman",12,"bold"),state="readonly")
        ...
        self.add_saleo = Button(self.top_frame, text="Add order", bg="#0A8F36",fg="white",font=("times new roman",12),width=10,
                                   command=lambda:self.mainfunction("A"))
        self.add_saleo.grid(row=0,column=5,padx=50)
        #middle frame
        ...
        #saleo table
        self.saleo_table =ttk.Treeview(self.middle_frame, columns=("oder_no",'date',"customer","bill",'deposite','balance',"duedate","status"
        ...
        horizont_scroll.config(command=self.saleo_table.xview)
        vertical_scroll.config(command=self.saleo_table.yview)
        self.saleo_table.heading('oder_no',text='Oder_no')
        self.saleo_table.heading('date',text='Date')
        self.saleo_table.heading('customer',text='Customer')
        self.saleo_table.heading('bill',text='Bill')
        self.saleo_table.heading('deposite',text='Deposit')
        self.saleo_table.heading('balance',text='Balance')
        self.saleo_table.heading('duedate',text='Due date')
        self.saleo_table.heading('status',text='Status')
        self.saleo_table.pack(pady=10)
        self.saleo_table.column('oder_no',width=80)
        self.saleo_table.column('date',width=100)
        self.saleo_table.column('customer',width=240)
        self.saleo_table.column('bill',width=180)
        self.saleo_table.column('deposite',width=150)
        self.saleo_table.column('balance',width=180)
        self.saleo_table.column('duedate',width=100)
        self.saleo_table.column('status',width=100)
    #met#bottom frame
        ...
        self.saleo_table.bind("<<TreeviewSelect>>",self.select)
        self.details_display()
    #methods
    ...
    def details_display(self):
        try:
            ...
            self.saleo_table.delete(*self.saleo_table.get_children())
            for value in data:
                self.saleo_table.insert("",END,values=value)
            ...
            if self.combo_box.get()!="Search by":
                if self.combo_box.get()=="Order_No":
                    if self.search_entry.get()!='':
                        query = '''SELECT*FROM sale_order WHERE o_id LIKE %s'''
                    ...
                    if self.search_entry.get()!='':
                        query = '''SELECT*FROM sale_order WHERE customer LIKE %s'''
                        mycursor.execute(query,f"%{self.search_entry.get()}%")
                        search =mycursor.fetchall()
                        if len(search)==0:
                        ...
                        query = '''SELECT*FROM sale_order WHERE date = %s'''
    ...
    def mainfunction(self,choice):
        if choice =='A':
            aowindow = Toplevel()
            obj = sale_odder(aowindow,self.root,self.details_display)
            aowindow.mainloop()
        elif choice =='c':
            con = self.database_connection(1996)
            mycursor = con.cursor()
            y = self.saleo_table.focus()
            y = self.saleo_table.item(y,"values")
            a=y[0]
            b =y[7]
            if b!="canceled" and b!="delivered" and b!="partially delivered":
                query1 = "UPDATE sale_order SET status =%s WHERE o_id = %s"
                if messagebox.askyesnocancel("","Are you sure you want to cancel this ordder?",parent=self.root):
                    value =("canceled",a)
                    mycursor.execute(query1,value)
                    extract ="SELECT p_id,qty FROM ordered_sale WHERE o_id =%s"
                    mycursor.execute(extract,a)
                    records =mycursor.fetchall()
                    for data in records:
                        mycursor.execute("SELECT qty FROM products WHERE p_id =%s",data[0])
                        retrev = mycursor.fetchone()
                        current =str(int(data[1].replace(",",""))+int(retrev[0].replace(",","")))
                        mycursor.execute("UPDATE products SET qty =%s WHERE p_id =%s",(current,data[0]))
                    con.commit()
            ...
            if oder[7] !="canceled":
                ...
                self.root1.geometry("800x630+350+30")
                ...
                lable =Label(self.root1,text=f"Customer: {oder[2]}",font=("times new roman",20,'bold'))
                lable.pack(fill=X)
                tab_value = oder[0]
                con = self.database_connection(1996)
                mycursor =  con.cursor()
                mycursor.execute("SELECT*FROM ordered_sale WHERE o_id =%s",oder[0])
                records = mycursor.fetchall()
                for record in records:
                    ...
                    Label(found,text="Qty delivered",bg="white").grid(row=0,column=2)
                    ...
                    found.pack(pady=4,anchor=W,padx=12)
                ...
                messagebox.showerror("Error","You cannot confirm an order that has been canceled!",parent=self.root)
                return
        elif choice == "d":
            y = self.saleo_table.focus()
            oder = self.saleo_table.item(y,"values")
            if oder[7]=="delivered" or oder[7]=="canceled": 
                ...
                if messagebox.askyesno("","Are you sure you want to permanently delete this order",parent=self.root):
                    try:
                        con =self.database_connection(1996)
                        mycursor = con.cursor()
                        mycursor.execute("DELETE FROM ordered_sale WHERE o_id =%s",tab)
                        con.commit()
                        mycursor.execute("DELETE FROM sale_order WHERE o_id =%s",tab)
                        con.commit()
                        ...
                        path = f"{non_os_partition}:/inventory/sale_order"
                ...
                messagebox.showerror("Error","You can only delete an order that has been fully delivered or canceled!",parent=self.root)
                return
    ...
    def control(self,root,choice):
        if choice =="ca":
            ...
            if messagebox.askyesno("","Do you really want to save changes",parent=root):
                try:
                    ...
                    for key,value in info.items():
                        con = self.database_connection(1996)
                        mycursor = con.cursor()
                        mycursor.execute("SELECT id,recieved,balance from ordered_sale WHERE p_id=%s AND o_id=%s",(value[0],value[1]))
                        ...
                        if int(fetch[2])<=0 and int(value[2][1])!=0:
                            ...
                            count+=bal
                            q="UPDATE ordered_sale SET recieved=%s,balance=%s WHERE id =%s"
                            d =(str(reciev),str(bal),fetch[0])                        
                    if count == 0:
                        status = "delivered"
                    else:
                    ...
                    mycursor.execute("SELECT balance,deposit FROM sale_order WHERE o_id=%s",tab_value)
                    fetch = mycursor.fetchone()
                    ...
                    mycursor.execute("UPDATE sale_order SET deposit=%s,balance=%s,status=%s WHERE o_id =%s",(f"{depo:,}",f"{to_in:,}",status,tab_value))
                    mycursor.execute(q,d)
                    con.commit()
                    #add the confirmed order to sales
                    mycursor.execute("SELECT*FROM sale_order WHERE o_id =%s",tab_value)
                    check = mycursor.fetchone()
                    if check[8] == "no":
                        id = "SELECT MAX(SUBSTR(s_id,1))FROM sales"
                        mycursor.execute(id)
                        ...
                        if check[5] == "0":
                            ...
                            statu ="outstanding balance"
                        querys = "INSERT INTO sales VALUES(%s,%s,%s,%s,%s,%s,%s)"
                        valuess =(new_id,f'{datetime.date.today()}',check[2],check[3],check[4],check[5],statu)
                        mycursor.execute(querys,valuess)
                        con.commit()
                        mycursor.execute("SELECT p_id,qty FROM ordered_sale WHERE o_id =%s",tab_value)
                        got = mycursor.fetchall()
                        for find in got:
                            queryi ="INSERT INTO sale_items(p_id,s_id,qty,unit_measure) VALUES(%s,%s,%s,(SELECT u_measure FROM products WHERE p_id =%s))"
                            valuei = (find[0],new_id,find[1],find[0])
                            mycursor.execute(queryi,valuei)
                            con.commit()
